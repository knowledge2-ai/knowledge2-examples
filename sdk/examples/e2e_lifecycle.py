...
def main() -> None:
    ...
    docs = [
        {
            ...
            "raw_text": (
                "Knowledge2 blends dense and sparse retrieval. Dense retrieval captures semantic "
    ...
    ingest_status = client.get_document_batch(corpus_id, ingest["batch_id"])
    print("Indexed docs:", len(ingest_status.get("doc_ids") or []))
    ...
    _wait_for_job(client, build["job_id"])
    ...
    optimize_job = _wait_for_job(client, optimize["job_id"])
    if optimize_job.get("status") != "succeeded":
        raise SystemExit(f"Optimize job failed with status {optimize_job.get('status')}")
