...
try:
    ...
    upload = client.upload_documents_batch_and_wait(
        corpus_id,
        docs,
        idempotency_key="batch-demo-abc123",
    )
    print("Upload batch:", upload.get("batch_id"))
    print("Upload job:", upload.get("job_id"))
    doc_ids = upload.get("doc_ids")
    print("Indexed docs:", len(doc_ids) if isinstance(doc_ids, list) else 0)
