"""Example 09b: Upload files/folders to K2, then use LangChain integration.

This is the most common workflow for apps:
1) Upload real files (single file or a whole directory).
2) Build indexes.
3) Retrieve / search / generate using LangChain adapters.
...
Run:
  ...
  python examples/09_langchain_k2_sdk/langchain_upload_files.py --file /path/to/a.pdf --file /path/to/b.md

Notes:
- File ingestion is done via the core SDK (`Knowledge2.upload_document(file_path=...)`).
- LangChain integrations cover retrieval + tool workflows.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload files/dirs to K2 and query via LangChain")
    ...
    if args.batch:
        ...
        for idx, path in enumerate(files, start=1):
            # Avoid leaking local absolute paths into source_uri.
            source_uri = f"file://{path.name}"
            resp = client.upload_document(
                corpus_id,
                file_path=str(path),
                source_uri=source_uri,
                metadata={"filename": path.name, "example": "langchain_upload_files"},
                auto_index=False,
            )
    ...
    retriever = K2LangChainRetriever(client=client, corpus_id=corpus_id, top_k=3)
    docs = retriever.invoke(args.query)
    print(f"\nRetriever returned {len(docs)} doc(s) for query={args.query!r}")
    for i, doc in enumerate(docs[:3], start=1):
        print(
            f"  {i:02d}. score={doc.metadata.get('score')} text={doc.page_content[:160].replace(chr(10), ' ')}"
        )

    tools = create_k2_langchain_tools(client=client, corpus_id=corpus_id, default_top_k=3)
    tool_map = {tool.name: tool for tool in tools}

    # Optional: grounded generation (requires server-side LLM config)
    try:
        gen = tool_map["k2_generate_answer"].invoke({"query": args.query, "top_k": 3})
